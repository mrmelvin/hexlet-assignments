...
public class AppTest {
    ...
    void getPerson() throws Exception {
        MockHttpServletResponse response = mockMvc.perform(get("/people/3")).andReturn().getResponse();
        assertThat(response.getStatus()).isEqualTo(200);
        // Проверяем, что тип содержимого в ответе JSON
        assertThat(response.getContentType()).isEqualTo(MediaType.APPLICATION_JSON.toString());
    ...
    void patchPerson() throws Exception {
        MockHttpServletResponse response = mockMvc
                                        .perform(
                                                patch("/people/4")
                                                        .contentType(MediaType.APPLICATION_JSON)
                                                        .content("{\"firstName\": \"Sam\", \"lastName\": \"Lloyd\"}"))
                                                    .andReturn()
                                                    .getResponse();
        assertThat(response.getStatus()).isEqualTo(200);


        MockHttpServletResponse responseGet = mockMvc.perform(get("/people/4")).andReturn().getResponse();

        assertThat(responseGet.getContentAsString()).contains("Sam", "Lloyd");
    }

    @Test
    void deletePerson() throws Exception {
        MockHttpServletResponse deleteResponse = mockMvc.perform(delete("/people/4"))
                                                        .andReturn().getResponse();

        assertThat(deleteResponse.getStatus()).isEqualTo(200);

        MockHttpServletResponse responseGetAll = mockMvc.perform(get("/people")).andReturn().getResponse();
        assertThat(responseGetAll.getContentAsString()).doesNotContain("Sam", "Lloyd");
    }
    // END
}
